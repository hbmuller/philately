...
**Parameters**: *label* [*string*] A textual label for the layer. Used mainly for searching for a layer and removing it.
...
**Parameters**: *x* and *y* [*number*] Numbers representing the the horizontal (x) and vertical (y) coordinates of the layer, relative to the canvas displayed size.
*More about that in the `RushEngine.setCanvasSize()` section below.*
...
**Parameters**: *layers* [*array*] An array of `RushLayer` instances to be rendered on the target canvas.
...
**Return value:** An integer indicating the new size of the layer stack. If an invalid *layer* parameter is provided, returns `false`.
...
**Parameters**: *layer* [*object* or *string*] An instance of the `RushLayer` class; or a non-empty string matching the "label" option of the layer(s) to be removed.
